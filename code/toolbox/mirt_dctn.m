...
isreala         = isreal(a);
k               = 1; 
if(~isreala), ia = imag(a); a = real(a); k=2; end; % check if complex
    
for i = 1:k
  a             = a(ind);  % rearange
  isgpu         = 0;       % GPU always SLOWER !!!
  if(and(exist('gpuDevice','file'),isempty(isgpu)))
    try
      gpnum     = gpuDevice;
      isgpu     = 1;
    catch
