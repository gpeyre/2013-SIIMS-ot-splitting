...
\eql{\label{eq-defn-interp}    
	...
	\left\{\begin{array}{cl}
		m_{i,j} &= (\bar m_{i+1,j} + \bar m_{i-1,j})/2,  \\
  		f_{i,j} &= (\bar f_{i,j+1/2} + \bar f_{i,j-1/2})/2.
\end{array}\right.
}
...
\eq{
	...
	\diverg(U)_{i,j} = 
	(\bar m_{i+1/2,j} - \bar m_{i-1/2,j}) + 
	(\bar f_{i,j+1/2} - \bar f_{i,j-1/2}).
}
...
\eq{
	b(U) = \left( (\bar m_{-\frac12,j}, \bar m_{N+\frac12,j})_{j=0}^{P}, (\bar f_{i,-1/2}, \bar f_{i,P+1/2} )_{i=0}^{N}\right)
